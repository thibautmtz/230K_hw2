...
def objective(trial):
    ...
    n_estimators = trial.suggest_int('n_estimators', 5, 15, step = 5)
...
class Logger:
    def __init__(self):
        self.filename = f'./outputs/optuna_training_catboost_rolling_window_{datetime.datetime.now().isoformat()}.csv'
...
for i, param_pair in enumerate(param_pairs):
    ...
    proposed_filename = f'output_catboost_{param_pair}_{datetime.datetime.now().isoformat()}'
