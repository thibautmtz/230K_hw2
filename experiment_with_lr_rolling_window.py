...
class Logger:
    def __init__(self):
        self.filename = f'./outputs/optuna_training_lr_rolling_window_{datetime.datetime.now().isoformat()}.csv'
...
study.optimize(objective, n_trials=1200, n_jobs = -1, timeout = 2 * 3600, show_progress_bar=True, callbacks = [Logger()])
...
for i, param_pair in enumerate(param_pairs):
    ...
    proposed_filename = f'output_lr_{param_pair}_{datetime.datetime.now().isoformat()}'
